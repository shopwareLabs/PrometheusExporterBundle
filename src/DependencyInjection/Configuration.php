        ...
        $rootNode
            ...
            ->children()
                ...
                ->arrayNode('endpoint')
                    ...
                    ->end()
                ->end()
                ->scalarNode('scrape_metrics_namespace')
                    ->defaultNull()
                    ->info('Prefix for scrape-time provider metrics: null inherits shopware.telemetry.metrics.namespace (same prefix as the stored metrics), an empty string disables the prefix, any other string is used as-is (sanitized)')
                ->end()
